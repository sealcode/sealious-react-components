...
function statefulCollection(component, ErrorClass) {
	...
	const StatefulCollectionMixin = React.createClass({
		...
		generateQuery: function() {
			...
			query.pagination.forward_buffer = 1; // load this more resources to check whether or not to display the "next" page

			query.sort = self.state.sort || self.props.sort;

			return query;
		},
		fetch: function(query, silent) {
			const self = this;
			...
			const get_promise = CachedHttp.get(
				...
				.then(response => {
					...
					response = response
						...
						.slice(0, query.pagination.items || Infinity);
		...
		refresh: function(force, silent) {
			// "silent" set to true means don't show that you're loading
			let query = this.generateQuery(this.props);
			this.fetch(query, silent);
		},
		componentDidMount: function() {
			const self = this;
			const flush_listener = () => {
				if (self.props.allow_flush) {
					self.refresh(true, true);
				}
			};
			Events.on("flush-all-caches", flush_listener);
			self.setState({ flush_listener: flush_listener });
			this.refresh();
		},
		componentWillUnmount: function() {
			const self = this;
			if (self.state.listener)
				Events.off("flush-all-caches", self.state.flush_listener);
		},
		...
		setFilterField: function(field_name, value) {
			...
			if (value === undefined || value === "") {
				...
				new_filter = merge(true, self.state.filter, {
					[field_name]: value
				});
			}
			self.setState({
		...
		render: function() {
			...
			if (this.state.error) {
				return React.createElement(ErrorClass, {
					error_data: this.state.error_data
				});
			}

			const child_props = merge(
				true,
				{
			...
			return React.createElement(component, child_props);
		}
	});
