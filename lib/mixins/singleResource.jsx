    ...
    return React.createClass({
        ...
        getTempBody: function(specification, resource) {
            ...
            for (var field_name in specification.fields) {
                temp_body[field_name] = resource.body[field_name] === undefined
                    ? ""
                    : resource.body[field_name];
            }
            return temp_body;
        },
        ...
        refresh: function() {
            ...
            CachedHttp.get(url, query, { responseType: "json", cache: true })
                .then(function(data) {
                    ...
                    const collection_name = parsed_url.pathname
                        .split("/")
                        .slice(-2)[0];
        ...
        update: function(e) {
            ...
            const request_body = Object.keys(temp_new_body).reduce(
                function(prev, next) {
            ...
            CachedHttp.flush();
            return rest
                .map("patch", url, fd, { cache: true })
                .catch(function(xml1, xml2, error) {
                    alert(errorToString(error));
                });
        },
        delete: function() {
            rest.delete(this.props.url, {}, { cache: true }).then(() => {
                ...
                if (e.preventDefault && e.target.type === "file") {
                    ...
                    reader.addEventListener(
                        ...
                        function() {
                            ...
                            self.setState({
                                temp_body: merge(true, self.state.temp_body, {
                                    [field_name]: file,
                                }),
                            });
                        },
        ...
        render: function() {
            ...
            if (this.state.loaded) {
                try {
                    const component_props = merge(true, this.props, {
